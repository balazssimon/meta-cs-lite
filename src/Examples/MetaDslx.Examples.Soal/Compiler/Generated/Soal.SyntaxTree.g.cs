...
namespace MetaDslx.Examples.Soal.Compiler
{
    ...
    /// </summary>
    public abstract partial class SoalSyntaxTree : SyntaxTree
    {
        ...
        /// </summary>
        public static SoalSyntaxTree Create(SoalSyntaxNode root, IncrementalParseData parseData, SoalParseOptions? options = null, string path = "", SourceText? text = null, Encoding? encoding = null, SourceHashAlgorithm checksumAlgorithm = SourceHashAlgorithm.Sha1)
        {
            if (root == null)
            {
                throw new ArgumentNullException(nameof(root));
            }
            var directives = root.Kind == SoalSyntaxKind.Main && root is ICompilationUnitRootSyntax compilationUnitRoot ?
        ...
        internal static SoalSyntaxTree CreateForDebugger(SoalSyntaxNode root, IncrementalParseData parseData, SourceText text, SoalParseOptions options)
        {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? SoalParseOptions.Default;
            var lexer = SoalLanguage.Instance.InternalSyntaxFactory.CreateLexer(text, options);
            using (var parser = SoalLanguage.Instance.InternalSyntaxFactory.CreateParser(lexer, oldParseData: IncrementalParseData.Empty, changes: null, cancellationToken: cancellationToken))
            {
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            var lexer = SoalLanguage.Instance.InternalSyntaxFactory.CreateLexer(newText, Options);
            using (var parser = SoalLanguage.Instance.InternalSyntaxFactory.CreateParser(lexer, oldTree?.ParseData ?? IncrementalParseData.Empty, changes))
            {
                var parseData = parser.Parse();
                var compilationUnit = (MainSyntax)parseData.RootNode;
                var tree = new ParsedSyntaxTree(newText, newText.Encoding, newText.ChecksumAlgorithm, this.FilePath, (SoalParseOptions)this.Options, compilationUnit, parseData);
        ...
        internal sealed class DummySyntaxTree : SoalSyntaxTree
        {
            ...
            {
                return Language.SyntaxFactory.MakeSyntaxTree((SoalSyntaxNode)root, options: (SoalParseOptions)options, path: FilePath, encoding: null);
            }
        ...
        private class ParsedSyntaxTree : SoalSyntaxTree
        {
            ...
            internal ParsedSyntaxTree(SourceText? textOpt, Encoding? encodingOpt, SourceHashAlgorithm checksumAlgorithm, string path, SoalParseOptions options, SoalSyntaxNode root, IncrementalParseData parseData, bool cloneRoot = true)
            {
                Debug.Assert(root != null);
        ...
        {
            public DebuggerSyntaxTree(SoalSyntaxNode root, IncrementalParseData parseData, SourceText text, SoalParseOptions options)
                : base(
                    text,
                    text.Encoding,
