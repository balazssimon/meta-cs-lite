...
{
    internal class MetaSymbolsClassifier : IClassifier
    {
        ...
        private SymbolSyntaxFacts _syntaxFacts;

        public MetaSymbolsClassifier(ITextBuffer textBuffer, MetaDslxMefServices mefServices)
        {
            ...
            _syntaxFacts = SymbolLanguage.Instance.SyntaxFacts;
        }
        ...
        private void ComputeClassifications(ITextSnapshot snapshot)
        {
            ...
            var lexer = new SymbolLexer(new Antlr4.Runtime.AntlrInputStream(text));
            ...
            while (token.Type > 0)
            {
                ...
                _classificationSpans.Add(new ClassificationSpan(new SnapshotSpan(snapshot, tokenSpan), GetClassificationType((SymbolSyntaxKind)AntlrSyntaxKind.FromAntlr(token.Type))));
                token = tokens.LT(1);
            }
        }

        private IClassificationType GetClassificationType(SymbolSyntaxKind syntaxKind)
        {
