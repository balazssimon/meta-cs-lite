...
{
    internal class MetaModelClassifier : IClassifier
    {
        ...
        public MetaModelClassifier(ITextBuffer textBuffer, MetaDslxMefServices mefServices)
        {
        ...
        private void ComputeClassifications(ITextSnapshot snapshot)
        {
            ...
            var lexer = new MetaLexer(new Antlr4.Runtime.AntlrInputStream(text));
            ...
            while (token.Type > 0)
            {
                ...
                _classificationSpans.Add(new ClassificationSpan(new SnapshotSpan(snapshot, tokenSpan), GetClassificationType((MetaSyntaxKind)AntlrSyntaxKind.FromAntlr(token.Type))));
                token = tokens.LT(1);
            }
        }

        private IClassificationType GetClassificationType(MetaSyntaxKind syntaxKind)
        {
