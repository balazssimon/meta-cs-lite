...
namespace MetaDslx.Languages.MetaCompiler.Symbols.Impl
{
    ...
    {
        public AnnotationArgumentSymbolImpl(Symbol? container, Compilation? compilation, MergedDeclaration? declaration, __IModelObject? modelObject, __ISymbol? csharpSymbol, ErrorSymbolInfo? errorInfo)
        ...
        protected override (DeclarationSymbol Parameter, MetaType ParameterType) Compute_Parameter(DiagnosticBag diagnostics, CancellationToken cancellationToken)
        {
            ...
            var slot = ModelObject.MGetSlot(MetaDslx.Languages.MetaCompiler.Model.Compiler.AnnotationArgument_Parameter);
            slot?.Add(param);
            ...
            {
                var typeSlot = ModelObject.MGetSlot(MetaDslx.Languages.MetaCompiler.Model.Compiler.AnnotationArgument_ParameterType);
                typeSlot?.Add(mtType);
            }
            else
            {
