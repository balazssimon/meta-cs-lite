...
namespace MetaDslx.Languages.MetaCompiler.Compiler
{
    ...
    /// <summary>
    /// The parsed representation of a Compiler source document.
    /// </summary>
    public abstract partial class CompilerSyntaxTree : SyntaxTree
    {
        ...
        /// </summary>
        public static CompilerSyntaxTree Create(CompilerSyntaxNode root, IncrementalParseData parseData, CompilerParseOptions? options = null, string path = "", SourceText? text = null, Encoding? encoding = null, SourceHashAlgorithm checksumAlgorithm = SourceHashAlgorithm.Sha1)
        {
            if (root == null)
            {
        ...
        internal static CompilerSyntaxTree CreateForDebugger(CompilerSyntaxNode root, IncrementalParseData parseData, SourceText text, CompilerParseOptions options)
        {
        ...
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(CompilerSyntaxNode, IncrementalParseData, CompilerParseOptions?, string, SourceText?, Encoding?, SourceHashAlgorithm)"/> method for creating a syntax tree.</para>
        /// </summary>
        internal static CompilerSyntaxTree CreateWithoutClone(CompilerSyntaxNode root, IncrementalParseData parseData)
        {
            Debug.Assert(root != null);
            return new ParsedSyntaxTree(
                textOpt: null,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? CompilerParseOptions.Default;
            var lexer = CompilerLanguage.Instance.InternalSyntaxFactory.CreateLexer(text, options);
            using (var parser = CompilerLanguage.Instance.InternalSyntaxFactory.CreateParser(lexer, oldParseData: IncrementalParseData.Empty, changes: null, cancellationToken: cancellationToken))
            {
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            var lexer = CompilerLanguage.Instance.InternalSyntaxFactory.CreateLexer(newText, Options);
            using (var parser = CompilerLanguage.Instance.InternalSyntaxFactory.CreateParser(lexer, oldTree?.ParseData ?? IncrementalParseData.Empty, changes))
            {
                var parseData = parser.Parse();
                var compilationUnit = (MainSyntax)parseData.RootNode;
                var tree = new ParsedSyntaxTree(newText, newText.Encoding, newText.ChecksumAlgorithm, this.FilePath, (CompilerParseOptions)this.Options, compilationUnit, parseData);
        ...
        {
            public DebuggerSyntaxTree(CompilerSyntaxNode root, IncrementalParseData parseData, SourceText text, CompilerParseOptions options)
                : base(
                    text,
                    text.Encoding,
