...
namespace MetaDslx.Languages.MetaModel.Compiler.Syntax
{
	...
	public sealed class MetaPropertySyntax : MetaSyntaxNode
	{
	    ...
	    public MetaPropertySyntax Update(SyntaxToken element, TypeReferenceSyntax type, PropertyNameSyntax name, PropertyOppositeSyntax propertyOpposite, SyntaxToken tSemicolon)
	    {
	        ...
	        {
	            var newNode = MetaLanguage.Instance.SyntaxFactory.MetaProperty(element, type, name, propertyOpposite, tSemicolon);
	            var annotations = this.GetAnnotations();
	            if (annotations != null && annotations.Length > 0)
	               newNode = newNode.WithAnnotations(annotations);
	...
	public sealed class PropertyOppositeSyntax : MetaSyntaxNode
	{
	    ...
	    public PropertyOppositeSyntax Update(SyntaxToken kOpposite, QualifierSyntax opposite)
	    {
	        ...
	        {
	            var newNode = MetaLanguage.Instance.SyntaxFactory.PropertyOpposite(kOpposite, opposite);
	            var annotations = this.GetAnnotations();
	            if (annotations != null && annotations.Length > 0)
	               newNode = newNode.WithAnnotations(annotations);
