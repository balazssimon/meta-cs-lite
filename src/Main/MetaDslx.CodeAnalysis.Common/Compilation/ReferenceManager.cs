...
namespace MetaDslx.CodeAnalysis
{
    public partial class Compilation
    {
        public sealed class ReferenceManager
        {
            ...
            {
                Debug.Assert(compilation._assemblyName == this.SimpleAssemblyName);
            }
            ...
            private bool CreateAndSetSourceAssemblyFullBind(Compilation compilation)
            {
                ...
                foreach (var reference in compilation._externalReferences.OfType<CSharpMetadataReference>())
                {
                    csharpReferences.Add(reference.CSharpReference);
                }
                var csharpCompilation = CSharpCompilation.Create(this.SimpleAssemblyName, references: csharpReferences.ToImmutableAndFree());
                ...
                foreach (var reference in compilation._externalReferences.OfType<MetaModelReference>())
                {
                    referencedModulesBuilder.Add(new MetaModelModuleSymbol(reference.MetaModel));
                }
                foreach (var reference in compilation._externalReferences.OfType<ModelReference>())
                {
