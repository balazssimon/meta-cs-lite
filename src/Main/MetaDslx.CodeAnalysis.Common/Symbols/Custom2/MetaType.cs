...
namespace MetaDslx.CodeAnalysis
{
    ...
    public struct MetaType : IEquatable<MetaType>
    {
        ...
        public bool IsModelObject => _original is IModelObject || OriginalTypeSymbol?.ModelObject is not null;
        ...
        public IModelObject? OriginalModelObject
        {
            get
            {
                ...
                return OriginalTypeSymbol?.ModelObject;
            }
        }
        ...
        public bool TryGetEnumValue(string name, out MetaSymbol value, DiagnosticBag diagnostics = null, CancellationToken cancellationToken = default)
        {
            ...
            if (csts is not null && (csts.TypeKind == TypeKind.Enum || csts.BaseType?.ToDisplayString(Microsoft.CodeAnalysis.SymbolDisplayFormat.FullyQualifiedFormat) == EnumFullName))
            {
                ...
                if (literal is not null)
                {
                    ...
                    var symbol = OriginalTypeSymbol.ContainingModule.SymbolFactory.GetSymbol<DeclarationSymbol>(literal, diagnostics, cancellationToken);
        ...
        public bool TryExtractNullableType(out MetaType innerType, DiagnosticBag diagnostics = null, CancellationToken cancellationToken = default)
        {
            ...
            if (csts is not null && csts.NullableAnnotation == NullableAnnotation.Annotated)
            {
                if (csts is INamedTypeSymbol nts && nts.Name == "Nullable" && nts.TypeArguments.Length == 1 && SymbolDisplayFormat.QualifiedNameOnlyFormat.ToString(OriginalTypeSymbol) == "System.Nullable")
                {
                    bool freeDiagnostics = diagnostics is null;
                    if (freeDiagnostics) diagnostics = DiagnosticBag.GetInstance();
                    var symbol = OriginalTypeSymbol.ContainingModule.SymbolFactory.GetSymbol<TypeSymbol>(nts.TypeArguments[0], diagnostics, cancellationToken);
                    if (freeDiagnostics) diagnostics.Free();
        ...
        public bool TryExtractCollectionType(out MetaType itemType, DiagnosticBag diagnostics = null, CancellationToken cancellationToken = default)
        {
            ...
            if (csts is not null && csts.IsGenericType && csts.TypeArguments.Length == 1)
            {
                bool freeDiagnostics = diagnostics is null;
                if (freeDiagnostics) diagnostics = DiagnosticBag.GetInstance();
                var symbol = OriginalTypeSymbol.ContainingModule.SymbolFactory.GetSymbol<TypeSymbol>(csts.TypeArguments[0], diagnostics, cancellationToken);
                if (freeDiagnostics) diagnostics.Free();
                if (symbol is not null && !symbol.IsErrorSymbol)
                {
