...
namespace Microsoft.CodeAnalysis.Text
{
    ...
    public abstract class SourceText
    {
        ...
        protected SourceText(ImmutableArray<byte> checksum = default(ImmutableArray<byte>), SourceHashAlgorithm checksumAlgorithm = SourceHashAlgorithm.Sha1, SourceTextContainer? container = null)
        {
            ...
            {
                throw new ArgumentException(ExceptionMessages.InvalidHash, nameof(checksum));
            }
        ...
        internal static void ValidateChecksumAlgorithm(SourceHashAlgorithm checksumAlgorithm)
        {
            ...
            {
                throw new ArgumentException(ExceptionMessages.UnsupportedHashAlgorithm, nameof(checksumAlgorithm));
            }
        }
            ...
            bool canBeEmbedded = false)
        {
            ...
            if (!stream.CanRead)
            {
                throw new ArgumentException(ExceptionMessages.StreamMustSupportReadAndSeek, nameof(stream));
            }
            ...
            try
            {
                ...
                foreach (var change in changes)
                {
                    if (change.Span.End > this.Length)
                        throw new ArgumentException(ExceptionMessages.ChangesMustBeWithinBoundsOfSourceText, nameof(changes));
                    ...
                    if (change.Span.Start < position)
                    {
                        ...
                        throw new ArgumentException(ExceptionMessages.ChangesMustNotOverlap, nameof(changes));
                    }
