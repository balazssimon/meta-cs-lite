...
namespace Microsoft.CodeAnalysis.Text
{
    ...
    public readonly struct TextSpan : IEquatable<TextSpan>, IComparable<TextSpan>
    {
        ...
        public static TextSpan FromBounds(int start, int end)
        {
            if (start < 0)
            {
                throw new ArgumentOutOfRangeException(nameof(start), ExceptionMessages.StartMustNotBeNegative);
            }

            if (end < start)
            {
                throw new ArgumentOutOfRangeException(nameof(end), ExceptionMessages.EndMustNotBeLessThanStart);
            }
