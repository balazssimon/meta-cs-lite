...
namespace MetaDslx.Languages.MetaSymbols.Compiler
{
    ...
    /// <summary>
    /// The parsed representation of a Symbol source document.
    /// </summary>
    public abstract partial class SymbolSyntaxTree : SyntaxTree
    {
        internal static readonly SymbolSyntaxTree Dummy = new DummySyntaxTree();
        ...
        /// </summary>
        public new abstract SymbolSyntaxNode GetRoot(CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// Gets the root node of the syntax tree if it is already available.
        /// </summary>
        public abstract bool TryGetRoot(out SymbolSyntaxNode root);
        ...
        /// </remarks>
        public new virtual Task<SymbolSyntaxNode> GetRootAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            SymbolSyntaxNode node;
        ...
        /// </summary>
        public static SymbolSyntaxTree Create(SymbolSyntaxNode root, IncrementalParseData parseData, SymbolParseOptions? options = null, string path = "", SourceText? text = null, Encoding? encoding = null, SourceHashAlgorithm checksumAlgorithm = SourceHashAlgorithm.Sha1)
        {
            if (root == null)
            {
                throw new ArgumentNullException(nameof(root));
            }
            var directives = root.Kind == SymbolSyntaxKind.Main && root is ICompilationUnitRootSyntax compilationUnitRoot ?
            ...
            return new ParsedSyntaxTree(
                ...
                path: path,
                options: options ?? SymbolParseOptions.Default,
                root: root,
        ...
        internal static SymbolSyntaxTree CreateForDebugger(SymbolSyntaxNode root, IncrementalParseData parseData, SourceText text, SymbolParseOptions options)
        {
        ...
        /// <summary>
        /// <para>
        /// Internal helper for <see cref="SymbolSyntaxNode"/> class to create a new syntax tree rooted at the given root node.
        /// This method does not create a clone of the given root, but instead preserves it's reference identity.
        /// </para>
        /// <para>NOTE: This method is only intended to be used from <see cref="SymbolSyntaxNode.SyntaxTree"/> property.</para>
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(SymbolSyntaxNode, CSharpParseOptions, string, Encoding)"/> method for creating a syntax tree.</para>
        /// </summary>
        internal static SymbolSyntaxTree CreateWithoutClone(SymbolSyntaxNode root, IncrementalParseData parseData)
        {
            Debug.Assert(root != null);
            return new ParsedSyntaxTree(
                textOpt: null,
                ...
                path: "",
                options: SymbolParseOptions.Default,
                root: root,
        ...
        /// </summary>
        public static SymbolSyntaxTree ParseText(
            string text,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? SymbolParseOptions.Default;
            var lexer = SymbolLanguage.Instance.InternalSyntaxFactory.CreateLexer(text, options);
            using (var parser = SymbolLanguage.Instance.InternalSyntaxFactory.CreateParser(lexer, oldParseData: IncrementalParseData.Empty, changes: null, cancellationToken: cancellationToken))
            {
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            var lexer = SymbolLanguage.Instance.InternalSyntaxFactory.CreateLexer(newText, Options);
            using (var parser = SymbolLanguage.Instance.InternalSyntaxFactory.CreateParser(lexer, oldTree?.ParseData ?? IncrementalParseData.Empty, changes))
            {
                var parseData = parser.Parse();
                var compilationUnit = (MainSyntax)parseData.RootNode;
                var tree = new ParsedSyntaxTree(newText, newText.Encoding, newText.ChecksumAlgorithm, this.FilePath, (SymbolParseOptions)this.Options, compilationUnit, parseData);
        ...
        internal sealed class DummySyntaxTree : SymbolSyntaxTree
        {
            private readonly SymbolSyntaxNode _node;
            ...
            {
                get { return SymbolParseOptions.Default; }
            }
            ...
            public override SymbolSyntaxNode GetRoot(CancellationToken cancellationToken)
            {
                return _node;
            }

            public override bool TryGetRoot(out SymbolSyntaxNode root)
            {
                root = _node;
                return true;
            }
            ...
            {
                return Language.SyntaxFactory.MakeSyntaxTree((SymbolSyntaxNode)root, options: (SymbolParseOptions)options, path: FilePath, encoding: null);
            }
        ...
        private class ParsedSyntaxTree : SymbolSyntaxTree
        {
            private readonly SymbolParseOptions _options;
            private readonly string _path;
            private readonly SymbolSyntaxNode _root;
            ...
            internal ParsedSyntaxTree(SourceText? textOpt, Encoding? encodingOpt, SourceHashAlgorithm checksumAlgorithm, string path, SymbolParseOptions options, SymbolSyntaxNode root, IncrementalParseData parseData, bool cloneRoot = true)
            {
                Debug.Assert(root != null);
                ...
                _hasCompilationUnitRoot = root.Kind == SymbolSyntaxKind.Main;
            }
            ...
            public override SyntaxTree WithRootAndOptions(SyntaxNode root, ParseOptions options)
            {
                ...
                return new ParsedSyntaxTree(
                    ...
                    _checksumAlgorithm,
                    _path,
                    (SymbolParseOptions)options,
                    (SymbolSyntaxNode)root,
                    this.ParseData);
            }
        ...
        {
            public DebuggerSyntaxTree(SymbolSyntaxNode root, IncrementalParseData parseData, SourceText text, SymbolParseOptions options)
                : base(
                    text,
                    text.Encoding,
