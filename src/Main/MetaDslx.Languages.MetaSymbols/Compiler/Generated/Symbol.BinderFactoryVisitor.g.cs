...
namespace MetaDslx.Languages.MetaSymbols.Compiler.Binding
{
    ...
    public class SymbolBinderFactoryVisitor : MetaDslx.CodeAnalysis.Binding.BinderFactoryVisitor, ISymbolSyntaxVisitor
    {
        ...
        {
            var __annot1 = new MetaDslx.CodeAnalysis.Binding.DefineBinder(type: typeof(MetaDslx.Languages.MetaSymbols.Model.Property));
        ...
        public virtual void VisitOperationAlt2(OperationAlt2Syntax node)
        {
            ...
            try
            {
                if (node.IsCached.GetSymbolKind() != SymbolSyntaxKind.None)
                {
                    var __annot0 = new MetaDslx.CodeAnalysis.Binding.PropertyBinder(name: "IsCached", values: ImmutableArray.Create<object?>(true));
                    this.Begin(__annot0, node.IsCached);
                    try
                    {
                        //this.VisitToken(node.IsCached);
                    }
                    finally
                    {
                        this.End(__annot0);
        ...
        public virtual void VisitSymbolBlock1(SymbolBlock1Syntax node)
        {
            ...
            try
            {
                var __annot0 = new MetaDslx.CodeAnalysis.Binding.UseBinder(types: ImmutableArray.Create<System.Type>(typeof(MetaDslx.Languages.MetaSymbols.Model.Symbol)), suffixes: ImmutableArray.Create<string>("", "Symbol"));
        ...
        {
            var __annot0 = new MetaDslx.CodeAnalysis.Binding.PropertyBinder(name: "IsPlain", values: ImmutableArray.Create<object?>(true));
            this.Begin(__annot0, node.IsPlain);
            try
            {
                //this.VisitToken(node.IsPlain);
            }
            finally
            {
                this.End(__annot0);
        ...
        public virtual void VisitPropertyBlock1Alt2(PropertyBlock1Alt2Syntax node)
        {
            ...
            if (node.IsCached.GetSymbolKind() != SymbolSyntaxKind.None)
            {
                var __annot1 = new MetaDslx.CodeAnalysis.Binding.PropertyBinder(name: "IsCached");
                this.Begin(__annot1, node.IsCached);
                try
                {
                    //this.VisitToken(node.IsCached);
                }
            ...
            {
                var __annot2 = new MetaDslx.CodeAnalysis.Binding.PropertyBinder(name: "IsWeak", values: ImmutableArray.Create<object?>(true));
                this.Begin(__annot2, node.IsWeak);
                try
                {
