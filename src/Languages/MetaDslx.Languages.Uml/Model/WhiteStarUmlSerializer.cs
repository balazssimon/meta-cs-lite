...
namespace MetaDslx.Languages.Uml.Model
{
    ...
    {
        public __Model ReadModelFromFile(string filePath)
        {
            ...
            {
                throw new ModelException(diagnostics[0].ToString());
            }
            return result;
        }

        public __Model ReadModel(string umlCode)
        {
        ...
        public __Model ReadModelFromFile(string filePath, out ImmutableArray<Diagnostic> diagnostics)
        {
            if (filePath == null) throw new ArgumentNullException(nameof(filePath));
            var umlCode = File.ReadAllText(filePath);
        ...
        public __Model ReadModel(string umlCode, out ImmutableArray<Diagnostic> diagnostics)
        {
            if (umlCode == null) throw new ArgumentNullException(nameof(umlCode));
            return this.ReadModel(string.Empty, umlCode, out diagnostics);
        }

        protected __Model ReadModel(string filePath, string umlCode, out ImmutableArray<Diagnostic> diagnostics)
        {
            if (umlCode == null) throw new ArgumentNullException(nameof(umlCode));
    ...
    internal class WhiteStarUmlReader
    {
        ...
        private __Model _model;
        private UmlModelFactory _factory;
        ...
        public WhiteStarUmlReader(string fileUri, string umlCode)
        {
            ...
            _umlCode = umlCode;
            _model = new __Model();
            ((__Model)_model).Name = _fileUri;
            _factory = new UmlModelFactory(_model);
        ...
        private ValueSpecification ParseDefaultValue(Type type, string value)
        {
            ...
            if (element.Name.LocalName == "OBJ" && element.Name.Namespace == _whiteStarUmlNamespace)
            {
                ...
                if (!SkipTypeName(typeName))
                {
                    ...
                    else
                    {
                        try
                        {
                            obj = (IModelObject)_factory.Create(typeName);
                        }
                    ...
                    if (obj != null)
                    {
                        ...
                        foreach (var nameProp in obj.MInfo.AllDeclaredProperties.Where(p => p.IsName))
                        {
                            ...
                            if (nameAttr != null)
                            {
                                ...
                                try
                                {
                                    obj.MGetSlot(nameProp)?.Add(name);
                                }
                                catch (ModelException mex)
                                {
                                    this.AddError(element, mex);
                ...
                if (parent != null)
                {
                    string parentPropertyName = GetPropertyName(parent.MInfo.MetaType.Name, nameAttribute?.Value);
                    if (parentPropertyName != null)
                    {
                        if (!HandleSpecialChild(element, parent, parentPropertyName, obj) && obj != null)
                        {
                            ...
                            {
                                this.AddWarning(element, $"Model object '{parent.MName}' ({parent.MInfo.MetaType.Name}) has no '{parentPropertyName}' property.");
                            }
                        }
                    }
                }
            ...
            if (parent != null && (element.Name.LocalName == "ATTR" || element.Name.LocalName == "REF") && element.Name.Namespace == _whiteStarUmlNamespace)
            {
                ...
                if (parentPropertyName != null)
                {
                    ...
                    if (!HandleSpecialProperty(element, parent, parentPropertyName, value))
                    {
                        ...
                        if (parentSlot != null)
                        {
                            this.AssignProperty(element, parent, parentSlot, value);
                        }
                        else
                        {
                            this.AddWarning(element, $"Model object '{parent.MName}' ({parent.MInfo.MetaType.Name}) has no '{parentPropertyName}' property.");
        ...
        private void AssignProperty(XObject location, IModelObject obj, ISlot slot, string propertyValue)
        {
            if (slot.Property.IsName) return;
            try
            {
                object value = ResolveValue(location, slot.Property.SlotProperty.Type, propertyValue);
                slot.Add(value);
            }
            catch (ModelException mex)
            {
        ...
        private bool HandleSpecialProperty(XObject location, IModelObject obj, string propertyName, string propertyValue)
        {
            ...
            if (obj is TypedElement typedElement)
            {
                if (propertyName == "TypeExpression")
                {
                    ...
                    {
                        var type = _model.Objects.OfType<Type>().Where(t => t.Name == propertyValue).FirstOrDefault();
                        if (type != null)
                        {
        ...
        private object ResolveValue(XObject location, MetaType type, string propertyValue)
        {
            if (type.IsAssignableTo(typeof(IModelObject)))
            {
                return this.ResolveObjectById(location, propertyValue);
            }
            else if (type.IsEnum)
            {
                if (type.TryGetEnumValue(propertyValue, out var enumValue))
                {
                    return enumValue;
                }
                else
