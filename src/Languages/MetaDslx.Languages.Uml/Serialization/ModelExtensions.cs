...
namespace MetaDslx.Languages.Uml.Serialization
{
    internal static class ModelExtensions
    {
        public static IEnumerable<Classifier> MBaseClassifiers(this Classifier cls)
        {
            ...
            result.UnionWith(((IModelObject)cls).Model.Objects.OfType<Realization>().Where(d => d.Client.Contains(cls)).SelectMany(d => d.Supplier).OfType<Classifier>());
            result.UnionWith(((IModelObject)cls).Model.Objects.OfType<Generalization>().Where(d => d.Specific == cls).Select(d => d.General).OfType<Classifier>());
            return result;
        }
