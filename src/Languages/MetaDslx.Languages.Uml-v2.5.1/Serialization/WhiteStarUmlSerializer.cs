...
namespace MetaDslx.Languages.Uml.Serialization
{
    ...
    public class WhiteStarUmlSerializer
    {
        public Model ReadModelFromFile(string filePath)
        {
            ...
            {
                throw new ModelException(diagnostics[0].GetMessage());
            }
            return result;
        }

        public Model ReadModel(string umlCode)
        {
        ...
        public Model ReadModelFromFile(string filePath, out ImmutableArray<Diagnostic> diagnostics)
        {
            if (filePath == null) throw new ArgumentNullException(nameof(filePath));
            var umlCode = File.ReadAllText(filePath);
        ...
        public Model ReadModel(string umlCode, out ImmutableArray<Diagnostic> diagnostics)
        {
            if (umlCode == null) throw new ArgumentNullException(nameof(umlCode));
            return this.ReadModel(string.Empty, umlCode, out diagnostics);
        }

        protected Model ReadModel(string filePath, string umlCode, out ImmutableArray<Diagnostic> diagnostics)
        {
            if (umlCode == null) throw new ArgumentNullException(nameof(umlCode));
    ...
    internal class WhiteStarUmlReader
    {
        ...
        private UmlMetaModelFactory _factory;
        ...
        private Dictionary<string, PrimitiveType> _primitiveTypes;
        private Dictionary<MessageOccurrenceSpecification, Lifeline> _mosToLifeline;
        private HashSet<string> _invisible;
        private Dictionary<Message, string> _messageLabels;
        ...
        public WhiteStarUmlReader(string fileUri, string umlCode)
        {
            ...
            _primitiveTypes = new Dictionary<string, PrimitiveType>();
            _mosToLifeline = new Dictionary<MessageOccurrenceSpecification, Lifeline>();
            _invisible = new HashSet<string>();
            _messageLabels = new Dictionary<Message, string>();
        }
        ...
        {
            _diagnostics.Add(Diagnostic.Create(ErrorCode.ERR_XmiError, GetLocation(location), message));
        }

        internal void AddError(XObject location, ModelException mex)
        {
            _diagnostics.Add(Diagnostic.Create(ErrorCode.ERR_XmiError, GetLocation(location), mex.Message));
        }

        internal void AddWarning(XObject location, string message)
        {
            _diagnostics.Add(Diagnostic.Create(ErrorCode.WRN_XmiWarning, GetLocation(location), message));
        }

        internal void AddWarning(XObject location, ModelException mex)
        {
            _diagnostics.Add(Diagnostic.Create(ErrorCode.WRN_XmiWarning, GetLocation(location), mex.Message));
        }
        ...
        private void ReadSequenceViews()
        {
            ...
            foreach (var interactionElement in interactionElements)
            {
                ...
                {
                    var interaction = ResolveObjectById(interactionElement, interactionId) as Interaction;
                    if (interaction != null)
                    {
                        var sequenceViews = new Dictionary<IModelObject, (int, int)>();
                        ...
                        foreach (var view in views)
                        {
                            ...
                            if (seqId != null)
                            {
                                ...
                                if (seqObj != null)
                                {
                                    if (viewType == "UMLSeqStimulusView" && seqObj is Message message)
                                    {
                                        ...
                                        if (!string.IsNullOrWhiteSpace(points))
                                        {
                                            ...
                                            if (pointItems.Length >= 4)
                                            {
                                                ...
                                                sequenceViews.Add((IModelObject)message.SendEvent, (start, start));
                                                sequenceViews.Add((IModelObject)message.ReceiveEvent, (end, end));
                                            }
                                        }
                                    ...
                                    else if (viewType == "UMLSeqObjectView" || viewType == "UMLCombinedFragmentView" || viewType == "UMLInteractionOperandView")
                                    {
                                        ...
                                        if (topString != null && heightString != null && int.TryParse(topString, out int top) && int.TryParse(heightString, out int height))
                                        {
                                            ...
                                            {
                                                if (seqObj is Lifeline)
                                                {
                                                    AddError(seqIdElement, "Duplicate lifeline '"+seqObj.Name+"' in interaction '"+ interaction.Name + "'");
                                                }
                                                else
                                                {
                                                    AddError(seqIdElement, "Duplicate element '" + seqObj.Name + "' in interaction '" + interaction.Name + "'");
                                                }
                                            }
                                            else
                                            {
                        ...
                        foreach (var view in sequenceViews.Where(v => v.Key is CombinedFragment).ToList())
                        {
                            var cf = (CombinedFragment)view.Key;
                            if (cf.Operand.Count > 0)
                            {
                                var firstOp = cf.Operand[0];
                                if (sequenceViews.TryGetValue((IModelObject)firstOp, out var firstRange))
                                {
                                    int top = Math.Min(view.Value.Item1, firstRange.Item1);
                                    sequenceViews[(IModelObject)firstOp] = (top, firstRange.Item2);
                                }
                                var lastOp = cf.Operand[cf.Operand.Count - 1];
                                if (sequenceViews.TryGetValue((IModelObject)lastOp, out var lastRange))
                                {
                                    int bottom = Math.Max(view.Value.Item2, lastRange.Item2);
                                    sequenceViews[(IModelObject)lastOp] = (lastRange.Item1, bottom);
                                }
                            }
                        }
                        var orderedViews = sequenceViews.OrderBy(entry => entry.Value.Item1).ThenByDescending(entry => entry.Value.Item2).ToList();
                        var fragmentStack = new Stack<KeyValuePair<IModelObject, (int, int)>>();
                        var lifelineMap = new Dictionary<Lifeline, Lifeline>();
                        foreach (var view in orderedViews)
                        {
                            if (view.Key is Lifeline lifeline)
                            {
                                if (((IModelObject)lifeline).Parent != null)
                                {
                                    ...
                                    if (_elementsByObject.TryGetValue((IModelObject)lifeline, out var lifelineElement))
                                    {
                                        _elementsByObject.Add((IModelObject)replacement, lifelineElement);
                                    }
                                }
                                else
                                {
                        ...
                        foreach (var view in orderedViews)
                        {
                            if (!(view.Key is Lifeline))
                            {
                                InteractionOperand currentFragment = null;
                                while (fragmentStack.Count > 0)
                                {
                                    ...
                                    {
                                        currentFragment = (InteractionOperand)top.Key;
                                        break;
                                    }
                                }
                                if (view.Key is CombinedFragment combinedFragment)
                                {
                                    ...
                                    {
                                        var location = ResolveElementByObject((IModelObject)combinedFragment, null);
                                ...
                                else if (view.Key is InteractionOperand interactionOperand)
                                {
                                    fragmentStack.Push(view);
                                }
                                else if (view.Key is MessageOccurrenceSpecification messageEnd)
                                {
                                    if (currentFragment != null)
                                    {
        ...
        {
            /*foreach (var dep in _model.Objects.OfType<Dependency>())
            {
                if (dep is InterfaceRealization intfReal)
                {
                    foreach (var client in intfReal.Client)
                    {
                        if (client is Class clientClass)
                        {
                            intfReal.Owner = null;
            ...
            foreach (var prop in _model.Objects.OfType<Property>())
            {
                var propElement = ResolveElementByObject((IModelObject)prop, null);
            ...
            foreach (var param in _model.Objects.OfType<Parameter>())
            {
                var paramElement = ResolveElementByObject((IModelObject)param, null);
            ...
            foreach (var assoc in _model.Objects.OfType<Association>())
            {
                if (assoc.MemberEnd.Count == 2)
                {
                    ...
                    if (firstNavigable)
                    {
                        ...
                        if (type is Class cls) cls.OwnedAttribute.Add(firstEnd);
                        else if (type is Interface intf) intf.OwnedAttribute.Add(firstEnd);
                    }
                    if (secondNavigable)
                    {
                        var type = firstEnd.Type;
                        if (type is Class cls) cls.OwnedAttribute.Add(secondEnd);
                        else if (type is Interface intf) intf.OwnedAttribute.Add(secondEnd);
                    }
                }
            }
            foreach (var inter in _model.Objects.OfType<Interaction>().ToList())
            {
                ...
                foreach (var lifeline in inter.Lifeline)
                {
                    ...
                    var lifelineElement = ResolveElementByObject((IModelObject)lifeline, null);
                    ...
                    {
                        classifier = ResolveObjectById(lifelineElement, classifierId) as Classifier;
                    }
                    else
                    {
                        classifier = _model.Objects.OfType<Class>().Where(c => c.Name == lifeline.Name).FirstOrDefault();
                        if (classifier == null)
                        {
                            classifier = _model.Objects.OfType<Interface>().Where(c => c.Name == lifeline.Name).FirstOrDefault();
                        }
                    }
                    if (classifier != null)
                    {
                ...
                {
                    var messageElement = ResolveElementByObject((IModelObject)message, null);
        ...
        private ValueSpecification ParseDefaultValue(MetaModel.Type type, string value)
        {
            ...
            if (type is Enumeration enumType)
            {
        ...
        private string ParseMessageArguments(Message message, XElement argumentsElement, XElement returnElement)
        {
        ...
        private void CreateObject(XElement element, IModelObject parent)
        {
            ...
            if (element.Name.LocalName == "OBJ" && element.Name.Namespace == _whiteStarUmlNamespace)
            {
                ...
                if (!SkipTypeName(typeName))
                {
                    ...
                    else
                    {
                        try
                        {
                            obj = (IModelObject)((IModelFactory)_factory).Create(typeName);
                        }
                ...
                if (parent != null)
                {
                    string parentPropertyName = GetPropertyName(parent.MetaType.Name, nameAttribute?.Value);
                    if (parentPropertyName != null)
                    {
                        if (!HandleSpecialChild(element, parent, parentPropertyName, obj) && obj != null)
                        {
                            ...
                            {
                                this.AddWarning(element, $"Model object '{parent.Name}' ({parent.MetaType.Name}) has no '{parentPropertyName}' property.");
                            }
                        }
                    }
                }
        ...
        private void AssignProperty(XObject location, IModelObject obj, ModelProperty property, string propertyValue)
        {
            if (property.IsName) return;
            try
            {
                object value = ResolveValue(location, property.Type, propertyValue);
        ...
        private bool HandleSpecialChild(XObject location, IModelObject parent, string parentPropertyName, IModelObject child)
        {
            if (parent is Package && child is Generalization generalization && parentPropertyName == "PackagedElement")
            {
                return true;
            }
            if (parent is Package && child is Include include && parentPropertyName == "PackagedElement")
            {
                return true;
            }
            if (parent is Package && child is Extend extend && parentPropertyName == "PackagedElement")
            {
                return true;
            }
            if (child is Interaction interaction)
            {
                ...
                if (parent is Package package) package.PackagedElement.Add(interaction);
                return true;
            }
            if (parent is Message message && parentPropertyName == "Action")
            {
                var element = (XElement)location;
        ...
        private bool HandleSpecialProperty(XObject location, IModelObject obj, string propertyName, string propertyValue)
        {
            if (obj is NamedElement namedElement)
            {
            ...
            if (obj is MultiplicityElement multiplicityElement)
            {
            ...
            if (obj is TypedElement typedElement)
            {
                if (propertyName == "TypeExpression")
                {
                    ...
                    if (openBracketIndex >= 0)
                    {
                        ...
                        if (closeBracketIndex >= 0)
                        {
                            ...
                            if (typedElement is MultiplicityElement multiplicityElement1)
                            {
                                var lowerValue = _factory.LiteralInteger();
                                lowerValue.Value = lower;
                                var upperValue = _factory.LiteralUnlimitedNatural();
                    ...
                    {
                        var type = _model.Objects.OfType<MetaModel.Type>().Where(t => t.Name == propertyValue).FirstOrDefault();
                        if (type != null)
                        {
            ...
            if (obj is InterfaceRealization interfaceRealization)
            {
            ...
            if (obj is Message message)
            {
                ...
                else if (propertyName == "Sender")
                {
                    ...
                    var lifeline = ResolveValue(location, typeof(IModelObject), propertyValue) as Lifeline;
            ...
            if (obj is CombinedFragment combinedFragment)
            {
            ...
            if (obj is InteractionOperand interactionOperand)
            {
