namespace MetaDslx.Bootstrap.MetaCompiler3.Symbols
{
    ...
    public abstract partial class PAlternativeSymbol: global::MetaDslx.CodeAnalysis.Symbols.Impl.DeclarationSymbolImpl
    {
        ...
        public ParserRuleSymbol? ContainingParserRuleSymbol
        {
            ...
            protected set
            {
                _containingParserRuleSymbol = value;
            }
        }
        [__PhaseAttribute]
        [__DerivedAttribute]
        ...
        protected override bool ForceCompletePart(ref __CompletionPart incompletePart, __SourceLocation? locationOpt, __CancellationToken cancellationToken)
        {
            if (incompletePart == CompletionParts.Start_ContainingParserRuleSymbol || incompletePart == CompletionParts.Finish_ContainingParserRuleSymbol)
            {
                ...
                {
                    if (_containingParserRuleSymbol == default)
                    {
            ...
            else if (incompletePart == CompletionParts.Start_ContainingPBlockSymbol || incompletePart == CompletionParts.Finish_ContainingPBlockSymbol)
            {
                ...
                {
                    if (_containingPBlockSymbol == default)
                    {
                        var diagnostics = __DiagnosticBag.GetInstance();
