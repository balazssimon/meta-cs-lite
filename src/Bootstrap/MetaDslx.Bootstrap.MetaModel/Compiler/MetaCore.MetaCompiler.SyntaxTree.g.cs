...
namespace MetaDslx.Bootstrap.MetaModel.Compiler
{
    ...
    /// <summary>
    /// The parsed representation of a MetaCore source document.
    /// </summary>
    public abstract partial class MetaCoreSyntaxTree : SyntaxTree
    {
        internal static readonly MetaCoreSyntaxTree Dummy = new DummySyntaxTree();
        ...
        /// </summary>
        public new abstract MetaCoreSyntaxNode GetRoot(CancellationToken cancellationToken = default(CancellationToken));

        /// <summary>
        /// Gets the root node of the syntax tree if it is already available.
        /// </summary>
        public abstract bool TryGetRoot(out MetaCoreSyntaxNode root);
        ...
        /// </remarks>
        public new virtual Task<MetaCoreSyntaxNode> GetRootAsync(CancellationToken cancellationToken = default(CancellationToken))
        {
            MetaCoreSyntaxNode node;
        ...
        /// </summary>
        public static MetaCoreSyntaxTree Create(MetaCoreSyntaxNode root, IncrementalParseData parseData, MetaCoreParseOptions? options = null, string path = "", SourceText? text = null, Encoding? encoding = null, SourceHashAlgorithm checksumAlgorithm = SourceHashAlgorithm.Sha1)
        {
            if (root == null)
            {
                throw new ArgumentNullException(nameof(root));
            }
            var directives = root.Kind == MetaCoreSyntaxKind.Main && root is ICompilationUnitRootSyntax compilationUnitRoot ?
            ...
            return new ParsedSyntaxTree(
                ...
                path: path,
                options: options ?? MetaCoreParseOptions.Default,
                root: root,
        ...
        internal static MetaCoreSyntaxTree CreateForDebugger(MetaCoreSyntaxNode root, IncrementalParseData parseData, SourceText text, MetaCoreParseOptions options)
        {
        ...
        /// <summary>
        /// <para>
        /// Internal helper for <see cref="MetaCoreSyntaxNode"/> class to create a new syntax tree rooted at the given root node.
        /// This method does not create a clone of the given root, but instead preserves it's reference identity.
        /// </para>
        /// <para>NOTE: This method is only intended to be used from <see cref="MetaCoreSyntaxNode.SyntaxTree"/> property.</para>
        /// <para>NOTE: Do not use this method elsewhere, instead use <see cref="Create(MetaCoreSyntaxNode, CSharpParseOptions, string, Encoding)"/> method for creating a syntax tree.</para>
        /// </summary>
        internal static MetaCoreSyntaxTree CreateWithoutClone(MetaCoreSyntaxNode root, IncrementalParseData parseData)
        {
            Debug.Assert(root != null);
            return new ParsedSyntaxTree(
                textOpt: null,
                ...
                path: "",
                options: MetaCoreParseOptions.Default,
                root: root,
        ...
        /// </summary>
        public static MetaCoreSyntaxTree ParseText(
            string text,
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            options = options ?? MetaCoreParseOptions.Default;
            var lexer = MetaCoreLanguage.Instance.InternalSyntaxFactory.CreateLexer(text, options);
            using (var parser = MetaCoreLanguage.Instance.InternalSyntaxFactory.CreateParser(lexer, oldParseData: IncrementalParseData.Empty, changes: null, cancellationToken: cancellationToken))
            {
        ...
        private SyntaxTree WithChanges(SourceText newText, IReadOnlyList<TextChangeRange> changes)
        {
            ...
            var lexer = MetaCoreLanguage.Instance.InternalSyntaxFactory.CreateLexer(newText, Options);
            using (var parser = MetaCoreLanguage.Instance.InternalSyntaxFactory.CreateParser(lexer, oldTree?.ParseData ?? IncrementalParseData.Empty, changes))
            {
                var parseData = parser.Parse();
                var compilationUnit = (MainSyntax)parseData.RootNode;
        ...
        internal sealed class DummySyntaxTree : MetaCoreSyntaxTree
        {
            private readonly MetaCoreSyntaxNode _node;
            ...
            public override MetaCoreSyntaxNode GetRoot(CancellationToken cancellationToken)
            {
                return _node;
            }
            public override bool TryGetRoot(out MetaCoreSyntaxNode root)
            {
                root = _node;
                return true;
            }
            ...
            {
                return Language.SyntaxFactory.MakeSyntaxTree((MetaCoreSyntaxNode)root, options: (MetaCoreParseOptions)options, path: FilePath, encoding: null);
            }
        ...
        private class ParsedSyntaxTree : MetaCoreSyntaxTree
        {
            private readonly MetaCoreParseOptions _options;
            private readonly string _path;
            private readonly MetaCoreSyntaxNode _root;
            ...
            internal ParsedSyntaxTree(SourceText? textOpt, Encoding? encodingOpt, SourceHashAlgorithm checksumAlgorithm, string path, MetaCoreParseOptions options, MetaCoreSyntaxNode root, IncrementalParseData parseData, bool cloneRoot = true)
            {
                Debug.Assert(root != null);
                ...
                _hasCompilationUnitRoot = root.Kind == MetaCoreSyntaxKind.Main;
            }
            ...
            public override SyntaxTree WithRootAndOptions(SyntaxNode root, ParseOptions options)
            {
                ...
                return new ParsedSyntaxTree(
                    ...
                    _checksumAlgorithm,
                    _path,
                    (MetaCoreParseOptions)options,
                    (MetaCoreSyntaxNode)root,
                    this.ParseData);
            }
            public override SyntaxTree WithFilePath(string path)
            {
        ...
        {
            public DebuggerSyntaxTree(MetaCoreSyntaxNode root, IncrementalParseData parseData, SourceText text, MetaCoreParseOptions options)
                : base(
                    text,
                    text.Encoding,
