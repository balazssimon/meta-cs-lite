...
//CompileWithMetaCompiler("Meta", @"..\..\..\..\..\Main\MetaDslx.Languages.MetaModel\Language", @"..\..\..\..\..\Main\MetaDslx.Languages.MetaModel\Compiler2", Meta.MInstance);
...
");
    ...
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.Language.g.cs"), languageCode);
    var languageVersionCode = generator.GenerateLanguageVersion();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.LanguageVersion.g.cs"), languageVersionCode);
    var parseOptionsCode = generator.GenerateParseOptions();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.ParseOptions.g.cs"), parseOptionsCode);
    var syntaxKindCode = generator.GenerateSyntaxKind();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.SyntaxKind.g.cs"), syntaxKindCode);
    var syntaxFactsCode = generator.GenerateSyntaxFacts();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.SyntaxFacts.g.cs"), syntaxFactsCode);
    var internalSyntaxCode = generator.GenerateInternalSyntax();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.InternalSyntax.g.cs"), internalSyntaxCode);
    var internalSyntaxVisitorCode = generator.GenerateInternalSyntaxVisitor();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.InternalSyntaxVisitor.g.cs"), internalSyntaxVisitorCode);
    var internalSyntaxFactoryCode = generator.GenerateInternalSyntaxFactory();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.InternalSyntaxFactory.g.cs"), internalSyntaxFactoryCode);
    var syntaxCode = generator.GenerateSyntax();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.Syntax.g.cs"), syntaxCode);
    var syntaxTreeCode = generator.GenerateSyntaxTree();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.SyntaxTree.g.cs"), syntaxTreeCode);
    var syntaxVisitorCode = generator.GenerateSyntaxVisitor();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.SyntaxVisitor.g.cs"), syntaxVisitorCode);
    var syntaxFactoryCode = generator.GenerateSyntaxFactory();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.SyntaxFactory.g.cs"), syntaxFactoryCode);
    var binderFactoryVisitorCode = generator.GenerateBinderFactoryVisitor();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.BinderFactoryVisitor.g.cs"), binderFactoryVisitorCode);
    var semanticsFactoryCode = generator.GenerateSemanticsFactory();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.SemanticsFactory.g.cs"), semanticsFactoryCode);
    var compilationFactoryCode = generator.GenerateCompilationFactory();
    File.WriteAllText(Path.Combine(outputDir, $"{lang}.MetaCompiler.CompilationFactory.g.cs"), compilationFactoryCode);
    var antlrDiagnostics = MetaDslx.CodeAnalysis.DiagnosticBag.GetInstance();
    var antlrCodes = generator.GenerateAntlr(name, antlrDiagnostics, default);
    ...
    else
    {
        ...
        {
            File.WriteAllText(Path.Combine(outputDir, antlrCode.FileName), antlrCode.Content);
        }
    }
